...
const NftModal: React.FC<NftModalProps> = ({ imageUrl, onClose, nftAttributes, nftName }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10 dark:bg-opacity-40">
      <div ref={modalRef} className="flex flex-col bg-white dark:bg-[#000A12] border dark:border-opacity-20 border-black dark:border-gray-800 dark:backdrop-filter dark:backdrop-blur-md dark:bg-opacity-80 text-black dark:text-white p-10 rounded-3xl relative xl:max-w-[80%] lg:max-w-[80%] md:max-w-[80%] w-[70%] max-h-[85%] overflow-auto">
          ...
          <div>
            ...
            <ul className="grid grid-cols-3 gap-4">
              {Object.entries(nftAttributes).map(([key, value]) => (
                <li key={key} className='bg-slate-100 dark:bg-[color:var(--secondary-color)] border dark:border-opacity-50 border-black dark:border-gray-600 p-3 mb-2 rounded-lg'>
