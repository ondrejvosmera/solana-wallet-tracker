...
export interface PriceBatched {
    ...
    mint: string;
    /**
     * The token price of the mint in its native units.
     *       > To convert to USDC ( stablecoin pegged to the US Dollar ), price * (10 ^ 6).
     */
    price: number;
    /**
     * The current volume, **sum of transactions** moved in the last minute by the token price.
     */
    volume: number;
    /**
...
export interface PriceBatchedRequestArgs {
    /**
     * Mint addresses per the SPL token program.  You must provide at least 1 and up to 100
     *         > You can also visit https://www.hellomoon.io/id?search=token to search for mints using a user
     * interface.
     */
    mints?: string[];
...
export declare class PriceBatchedRequest extends BaseRequest<PriceBatchedRequestArgs, PaginatedResponse<PriceBatched>> {
    readonly body: PriceBatchedRequestArgs;
    readonly method = "POST";
    readonly url = "/v0/token/price/batched";
    constructor(body?: PriceBatchedRequestArgs);
}
